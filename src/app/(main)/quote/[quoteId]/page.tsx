...
import { Loader2, ArrowLeft, Calendar, User, Briefcase, Hash, FileText, Layers, CheckCircle } from 'lucide-react';
                    ...
                    <div className="bg-gray-50 p-6 flex justify-between items-center">
                        ...
                        <div className="flex gap-3">
                            {isClient && quote.status === 'PENDING' && (
                                <>
                                    <Button variant="outline" onClick={() => updateQuoteStatus.mutate({ id: quote.id, status: 'REJECTED' })} disabled={updateQuoteStatus.isPending || acceptQuote.isPending}>Decline</Button>
                                    <Button onClick={() => acceptQuote.mutate({ id: quote.id })} disabled={acceptQuote.isPending}>
                                        {acceptQuote.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <CheckCircle className="mr-2 h-4 w-4" />}
                                        Accept Quote
                                    </Button>
                                </>
                            )}
