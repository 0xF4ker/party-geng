...
import { Loader2, ArrowLeft, Calendar, User, Briefcase, Hash, FileText, Layers, ShoppingCart } from 'lucide-react';
...
const QuoteDetailPage = () => {
    ...
    const handleAddToCart = async () => {
        if (!quote) return;
        const promise = addItem({ type: 'QUOTE', quoteId: quote.id });
        toast.promise(promise, {
            loading: 'Adding to cart...',
            success: 'Quote added to cart!',
            error: (err) => err instanceof Error ? err.message : 'Failed to add to cart.',
        });
    }
                        ...
                        <div className="flex gap-3">
                            {isClient && quote.status === 'PENDING' && (
                                <>
                                    ...
                                    <Button onClick={handleAddToCart}><ShoppingCart className="mr-2 h-4 w-4" />Add to Cart</Button>
                                </>
                            )}
