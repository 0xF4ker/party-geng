...
export async function GET(
  ...
  return new ImageResponse(
    (
      ...
      >
        <div style={{ display: 'flex', alignSelf: 'flex-start', alignItems: 'center' }}>
            {/* Using a placeholder for the logo */}
            <img src={new URL('/logo.png', request.nextUrl.origin).toString()} width="150" alt="logo" />
        </div>
        ...
        {wishlistItems.length > 0 && (
          <div
            style={{ display: "flex", flexDirection: "column", fontSize: 24, alignItems: 'center' }}
          >
            <span
            ...
            <div style={{ display: "flex", gap: 20, marginTop: 10 }}>
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.5)', padding: '5px 15px', borderRadius: '20px' }}>
                    <span style={{ marginRight: 8, fontSize: 20 }}>{item.itemType === 'CASH_REQUEST' ? '💰' : '🎁'}</span>
                    <span>{item.name}</span>
                </div>
                ))}
            </div>
