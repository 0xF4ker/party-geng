      ...
      <div className="flex items-center justify-between py-2">
        ...
        <div className="flex items-center gap-2">
           <div className="h-8 w-8 rounded-full bg-gray-100 overflow-hidden border border-white shadow-sm">
             {order.client.clientProfile?.avatarUrl && <img src={order.client.clientProfile.avatarUrl} className="h-full w-full object-cover" />}
           </div>
           <div className="text-xs">
             <span className="block text-gray-500">Client</span>
             <span className="font-medium text-gray-900 max-w-[80px] truncate block">
        ...
        <div className="flex items-center gap-2 flex-row-reverse text-right">
           <div className="h-8 w-8 rounded-full bg-gray-100 overflow-hidden border border-white shadow-sm">
             {order.vendor.vendorProfile?.avatarUrl && <img src={order.vendor.vendorProfile.avatarUrl} className="h-full w-full object-cover" />}
           </div>
           <div className="text-xs">
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium ${styles[status as keyof typeof styles]}`}>
...
export default function AdminOrdersPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 2. Controls Section (Stacks on mobile, Row on Desktop) */}
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
         ...
         <div className="relative w-full sm:w-64">
           ...
           <input 
             ...
             className="h-10 w-full rounded-lg border border-gray-200 pl-10 pr-4 text-sm focus:border-pink-500 focus:outline-none focus:ring-1 focus:ring-pink-500"
             value={search}
         ...
         <div className="relative w-full sm:w-48">
           ...
           <select 
             className="h-10 w-full appearance-none rounded-lg border border-gray-200 bg-white pl-10 pr-8 text-sm outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500"
             value={statusFilter || ""}
                 ...
                 <tr key={order.id} className="group hover:bg-gray-50/50">
                    ...
                    <td className="px-6 py-4">
                       <div className="flex items-center gap-2">
                         ...
                         <span className="font-medium text-gray-900">{order.client.clientProfile?.name || order.client.username}</span>
                       </div>
                    </td>
                    <td className="px-6 py-4">
                       <div className="flex items-center gap-2">
                         <Store className="h-3 w-3 text-gray-400" />
                         <span>{order.vendor.vendorProfile?.companyName || order.vendor.username}</span>
                       </div>
                    </td>
     ...
     <div className="grid gap-4 md:hidden">
      ...
      {/* --- IMPROVED DETAIL SHEET --- */}
<Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
  <SheetContent className="flex h-full w-full flex-col border-l border-gray-200 bg-white p-0 sm:max-w-xl shadow-2xl">
    ...
    {selectedOrder && (
      <div className="flex-1 overflow-y-auto px-8 py-8">
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-gray-900">
                      {selectedOrder.client.clientProfile?.name || selectedOrder.client.username}
                    </p>
                    <p className="truncate text-xs text-gray-500">{selectedOrder.client.email}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3 rounded-2xl border border-gray-100 bg-gray-50/50 p-4 transition-colors hover:border-blue-100 hover:bg-blue-50/30">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-gray-900">
                      {selectedOrder.vendor.vendorProfile?.companyName || selectedOrder.vendor.username}
                    </p>
                    <p className="truncate text-xs text-gray-500">{selectedOrder.vendor.email}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 divide-x divide-gray-100">
                <div className="p-4">
                  ...
                  <div className="mt-1 flex items-baseline gap-1">
                    ...
                    <span className="text-xs font-medium text-gray-400">NGN</span>
                  </div>
                </div>
                <div className="p-4">
                  <span className="block text-xs text-gray-500">Scheduled Date</span>
                  <div className="mt-1 flex items-center gap-2 font-medium text-gray-900">
                    <Calendar className="h-4 w-4 text-pink-600" />
                    {format(new Date(selectedOrder.quote.eventDate), "MMM d, yyyy")}
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* SECTION: Admin Control Panel */}
          <div className="space-y-3">
            <h4 className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-orange-600">
            ...
            <div className="rounded-2xl border border-orange-100 bg-orange-50/40 p-5">
              <p className="mb-4 text-sm text-gray-600 leading-relaxed">
                Use these actions to resolve disputes or correct system errors. 
                <span className="font-semibold"> These actions bypass the standard user flow.</span>
              </p>
              ...
              <div className="flex flex-col gap-3">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    <span className="text-sm font-semibold text-gray-900">Mark Dispute</span>
                    <span className="text-xs text-gray-500 font-normal">Pause transaction</span>
                  </Button>
                  ...
                  >
                    <span className="text-sm font-semibold text-red-700">Force Cancel</span>
                    <span className="text-xs text-red-600/70 font-normal">Refund client</span>
                  </Button>
                </div>
                
                <Button
                  className="h-auto w-full items-center justify-between bg-emerald-600 p-3 hover:bg-emerald-700 text-white shadow-sm"
                  onClick={() => handleStatusChange("COMPLETED")}
                >
                   <span className="flex flex-col items-start">
                      <span className="font-semibold">Force Complete Order</span>
                      <span className="text-xs opacity-90 font-normal">Release funds to vendor</span>
                   </span>
    ...
    {/* 3. FOOTER (Optional, if needed for 'Close' or primary action) */}
