...
export function AdminUsersTable({ initialRole }: AdminUsersTableProps) {
  ...
  const [activeAction, setActiveAction] = useState<"DELETE" | "SUSPEND" | "ROLE" | null>(null);
  ...
  const openModal = (action: "DELETE" | "SUSPEND" | "ROLE", user: any) => {
...
function DesktopUserRow({ user, onView, onAction, onRestore }: any) {
  const displayName = user.clientProfile?.name || user.vendorProfile?.companyName || user.username;
  const displayAvatar = user.clientProfile?.avatarUrl || user.vendorProfile?.avatarUrl;
  ...
  return (
    <tr className="group hover:bg-gray-50/50">
      ...
      <td className="px-6 py-4">
         <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium ${user.status === 'ACTIVE' ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}>
      ...
      <td className="px-6 py-4 text-right">
        <div className="flex justify-end gap-2">
           ...
           <UserActionsDropdown user={user} onAction={onAction} onRestore={onRestore} isSuspended={isSuspended} />
        </div>
      </td>
    </tr>
  );
...
function MobileUserCard({ user, onView, onAction }: any) {
  const displayName = user.clientProfile?.name || user.vendorProfile?.companyName || user.username;
  const displayAvatar = user.clientProfile?.avatarUrl || user.vendorProfile?.avatarUrl;
...
function UserActionsDropdown({ user, onAction, onRestore, isSuspended }: any) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        {isSuspended ? (
          ...
          <DropdownMenuItem onClick={() => onAction("SUSPEND", user)} className="text-orange-600">
            <Ban className="mr-2 h-4 w-4" /> Suspend User
          </DropdownMenuItem>
        )}
        <DropdownMenuSeparator />
        <DropdownMenuItem onClick={() => onAction("DELETE", user)} className="text-red-600">
